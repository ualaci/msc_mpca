...
\institute{Programa de Pós-Graduação em Computação Aplicada (PPCOMP), Instituto Federal do Espírito Santo (IFES) \& TSA Lynx Proccess}
\date{\today}
...
\begin{frame}{Proposta de Parceria: Objetivos e Impactos Estratégicos 2}
    \begin{block}{Etapa 2: MVP Comparativo (Aplicação Direta)}
        ...
        \textbf{Comparativo com a Infraestrutura Atual (Opcional):} O MVP será submetido a redes estocásticas degradadas (simulando instabilidade em conexões móveis). Suas métricas de latência e consumo de CPU podem ser diretamente comparadas com a stack padrão baseada em \textit{ROS} (tecnologia fundamental de sistemas como a atual \textit{VAL Stack}).\\ \vspace{1.5ex}

        \textbf{Segurança e \textit{Future-Proofing}:} O resultado fornece à empresa a validação matemática de determinismo temporal. Isso garante extrema confiabilidade para operações de missão crítica, como teleoperação de veículos em áreas com perda de sinal, pavimentando a evolução segura da frota.
    \end{block}
\end{frame}
